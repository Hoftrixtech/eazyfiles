...
export interface CropClientResult {
  blob: Blob;
  objectUrl: string;
  filename: string;
  originalSize: number;
...
function filenameFromDisposition(header: string | null): string {
  ...
  return safeDownloadName(asciiMatch?.[1] ?? "cropped-image");
}
...
export async function cropImageRequest(input: {
  ...
  height: number;
  outputFormat: string;
}): Promise<CropClientResult> {
  const body = new FormData();
  body.append("file", input.file);
  body.append("x", String(input.x));
  body.append("y", String(input.y));
  ...
  try {
    response = await fetch("/api/image-cropper", {
      method: "POST",
      body,
    ...
    throw new Error("We could not reach the crop service. Check your connection and try again.");
  }
  ...
  if (!response.ok) {
    ...
    throwFromApiPayload(payload, "Cropping failed. Please try a different image or crop area.");
  }
