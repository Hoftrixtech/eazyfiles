...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (!(fileValue instanceof File) || fileValue.size === 0) {
      throw new AppError("MISSING_FILE", "Please choose an image to crop.", 400);
    }
    ...
    let reserved;
    try {
      reserved = await reserveToolAccess("image-cropper", identity);
    } catch (error) {
    ...
    const result = await withTempDirectory(async (directory) => {
      ...
      return { cropped, outputBuffer };
    });

    if (
      result.outputBuffer.length === 0 ||
      detectImageFormat(result.outputBuffer) !== result.cropped.outputFormat
    ) {
      throw new AppError(
        "CROP_FAILED",
        "Cropping failed. Please try again with a different image or crop.",
        500
      );
    }

    const filename = `${sanitizeDownloadBase(fileValue.name)}-cropped.${FORMAT_TO_EXTENSION[result.cropped.outputFormat]}`;

    const response = new Response(new Uint8Array(result.outputBuffer), {
      status: 200,
      headers: {
        "Content-Type": FORMAT_TO_MIME[result.cropped.outputFormat],
        "Content-Disposition": attachmentDisposition(filename),
        "X-Content-Type-Options": "nosniff",
        "Cache-Control": "no-store",
        "X-Success": "1",
        "X-Original-Size": String(contents.byteLength),
        "X-Output-Size": String(result.cropped.outputBytes),
        "X-Original-Width": String(result.cropped.originalWidth),
        "X-Original-Height": String(result.cropped.originalHeight),
        "X-Output-Width": String(result.cropped.outputWidth),
        "X-Output-Height": String(result.cropped.outputHeight),
        "X-Output-Format": result.cropped.outputFormat,
        "X-Size-Delta-Percent": String(result.cropped.sizeDeltaPercent),
